...
28. **OpenAPI supplement envelope mismatch after factory migration:** When migrating a route from v1 factory (`createXxxApiHandlerWithAuth`) to v2 factory (`createAxiomRouteHandler(withAuth(...))`), the response format changes from `{data: T, error: null}` envelope to bare `T`. The OpenAPI supplement examples MUST be updated to match. Success examples: strip `data:` wrapper and `error: null` — use bare value (e.g., `value: { status: "completed" }` not `value: { data: { status: "completed" }, error: null }`). Error examples: strip `data: null` — use `value: { error: "message" }` only. Also update any `description` strings that mention the old envelope format. The scanner handles schemas automatically via `contract: "v2"`, but examples are manually curated and will NOT auto-update.
