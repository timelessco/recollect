...
28. **[SUPERSEDED 2026-04-15 — see `.claude/rules/api-v2.md` Auth Patterns]** ~~v1 routes that manually call `createApiClient()` are still `withPublic`.~~ The factory choice is still correct (v1 `createGetApiHandler` → v2 `withPublic`), but the client choice has changed: v2 `withPublic` now defaults to `createServerServiceClient()` + explicit handler-side gating, NOT `createApiClient()`. RLS coverage in this codebase is incomplete (`categories` and `bookmark_tags` have no anon policies; `everything.anon_discover_access` uses `trash = false` against a `timestamptz` column post-migration). Use `createApiClient()` only for the conditional-auth pattern in pitfall #34. After migration, `.config.auth` should still be `"none"` for `withPublic` routes regardless of which client they construct.
...
34. **`withPublic` + conditional `getApiUser()` is only justified when one URL serves two audiences via a request-time discriminator:** `withPublic` leaves auth to the handler — reaching for it just to "do auth manually" is almost always wrong. Default to `withAuth` (always-authed) or `withPublic` + `createServerServiceClient()` + explicit handler-side gating (always-public, RLS bypassed). The legitimate conditional-auth case is a single endpoint whose audience is decided by an input param: `search-bookmarks` switches on `category_id === DISCOVER_URL` — the discover branch runs anon, every other branch calls `getApiUser()` for `user_id` scoping and `isUserOwnerOrAnyCollaborator` checks. Splitting into two routes would duplicate the shared RPC + token-parsing pipeline; `withAuth` would 401 the public branch; a service client would strip the identity the authed branch needs. Before reaching for this pattern, confirm: (a) both branches share substantial logic, (b) the discriminator is a server-known constant or validated input — never a forgeable header, (c) the public branch exposes no per-user data even when the discriminator is spoofed.
