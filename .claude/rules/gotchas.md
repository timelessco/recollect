...
- `environment.d.ts` global augmentation was removed — `process.env.X` is now `string | undefined` by default. Use `env.X` from `@/env/server` or `@/env/client` for typed access
