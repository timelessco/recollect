...
| `withRawBody` | None (`auth` field is metadata only) | `{ request, route }` | Queue consumers, FormData uploads |
| `withSecret` | Timing-safe bearer token | `{ data, route }` | Internal-only endpoints |

**Return:** Raw data -- factory validates via `outputSchema.safeParse()` and wraps in `NextResponse.json()`.
Exception: `withRawBody` handlers return `NextResponse` directly.
...
throw new RecollectApiError("service_unavailable", {
  ...
  operation: "some_op",    // optional -- snake_case identifier for Axiom search
  context: { ... },        // optional -- extra structured context (e.g., Zod issues)
});
```
...
- ALWAYS pass `cause` when wrapping a caught error (DB error, fetch error, storage error)
- OMIT `cause` when the error is a business logic condition with no underlying error (access denied, not found, validation)
- For helper functions that rethrow, use `new Error(msg, { cause: error })` (ES2022) to preserve the chain
...
- Never string-concatenate error messages -- use `cause` instead
...
- **Unknown errors** (non-RecollectApiError): outer layer catches -> Axiom `error` -> re-throw -> `onRequestError` -> **Sentry**
...
- No direct `logger.info()` calls in handler body -- business context flows via `ctx.fields` only
...
- Register `after()` BEFORE throwing if enrichment should run on failure path too

### Response Patterns

- Success: `return data` -- factory wraps in `NextResponse.json()`
- `NextResponse.json()` escape hatch: dynamic RPC results, FormData routes (`withRawBody`), or redirects
...
- **Inline helpers** (server-safe, avoid importing client modules): return `null` on failure, log to `ctx.fields`
- **Imported helpers**: throw `RecollectApiError` directly -- propagates cleanly to inner layer catch
...
- Object.Assign routes (`get-media-type`, `get-pdf-buffer`) use raw `NextResponse.json` for errors -- NOT `apiError`/`apiWarn`. Expected pattern per migrator Section 4 template
