			            ...
			            ref(qualifiedName: $branchName) {
			              target { ... on Commit {
			                  history(first: $pageSize, since: $since ${afterCursorString}) {
			                    nodes { oid, author { user { login }}, associatedPullRequests(first: 1) { nodes { number, title }}}
...
export async function getOldestCommitSinceLastTag() {
  try {
    ...
    const { stdout: latestTag } = await execa("git", ["describe", "--tags", "--abbrev=0"]).catch(
      () => ({ stdout: "" }),
    );
    ...
    if (!stdout) {
      ...
      const tagOutput = await execa("git", ["log", "-1", "--format=%h %aI", latestTag]).catch(
        () => ({ stdout: "" }),
      );
      if (tagOutput.stdout) {
    ...
    const [, commitHash, commitDate] = /^"?([\da-f]+)\s([\d+:T\\|-]*)"?$/u.exec(commitResult) || [];
      ...
      for (const commit of historyData.nodes) {
        if (commit?.oid && commit?.author) {
          remoteCommits.push({
            ...
            prNumber: commit?.associatedPullRequests?.nodes?.[0]?.number ?? null,
            prTitle: commit?.associatedPullRequests?.nodes?.[0]?.title ?? null,
