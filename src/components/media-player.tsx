...
export function MediaPlayer({ isActive, mediaType, onError, src, title }: MediaPlayerProps) {
  if (mediaType === "audio") {
    return (
      <Suspense
        fallback={
      ...
      >
        <AudioWaveformPlayer isActive={isActive} onError={onError} src={src} title={title} />
      </Suspense>
    );
  }
  ...
  return (
    <MediaProvider>
      <VideoPlayerInner isActive={isActive} mediaType={mediaType} onError={onError} src={src} />
    </MediaProvider>
  );
}
