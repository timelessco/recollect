...
export const useMiscellaneousStore = create<MiscellaneousStoreState>((set) => ({
  ...
  lightboxId: null,
  setLightboxId: (id: string | null) => set((state) => ({ ...state, lightboxId: id })),
  lightboxOpen: false,
  setLightboxOpen: (open: boolean) => set((state) => ({ ...state, lightboxOpen: open })),
  shareCategoryId: undefined,
  setShareCategoryId: (id: number | undefined) => set(() => ({ shareCategoryId: id })),
  lightboxShowSidepane: false,
  setLightboxShowSidepane: (value: boolean) => set(() => ({ lightboxShowSidepane: value })),
  searchText: "",
  setSearchText: (value: string) => set(() => ({ searchText: value })),
  triggerHeadingEdit: false,
  setTriggerHeadingEdit: (value: boolean) => set(() => ({ triggerHeadingEdit: value })),
  ...
  setSelectedVideoId: (value: number | null) => set(() => ({ selectedVideoId: value })),
  renderedBookmarks: {},
  ...
  setDeleteBookmarkId: (bookmarkIds: number[]) => set(() => ({ deleteBookmarkId: bookmarkIds })),
}));
