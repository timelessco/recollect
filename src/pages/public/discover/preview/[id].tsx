...
import { buildPublicDiscoverUrl } from "../../../../utils/url-builders";

const PublicDiscoverPreviewParamsSchema = z.object({
  id: z.string().regex(/^\d+$/u, "Bookmark ID must be numeric").transform(Number),
});

export interface PublicDiscoverPreviewProps {
  bookmark: SingleListData;
}

const PublicDiscoverPreview = (props: PublicDiscoverPreviewProps) => {
...
export const getStaticProps: GetStaticProps<PublicDiscoverPreviewProps> = async (context) => {
  const ROUTE = "/public/discover/preview/[id]";

  const validation = PublicDiscoverPreviewParamsSchema.safeParse(context.params);
...
export default PublicDiscoverPreview;
