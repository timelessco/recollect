...
export default async function handler(
	...
	try {
		...
		if (mediaType && mediaType === PDF_MIME_TYPE) {
			...
			const { data: screenshotData } = await axios.post(
				"http://5.223.51.64/upload/pdf-screenshot",
				{ url },
				{
					headers: {
			...
			publicURL = screenshotData?.publicUrl;
		} else {
			console.log(
				"######################## Screenshot Loading ########################",
			);
			try {
				...
				console.log("aadadsfasdF", screenshotData);
