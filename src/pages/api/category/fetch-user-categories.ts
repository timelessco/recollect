...
export default async function handler(
  ...
  try {
    ...
    if (!supabase) {
      ...
      Sentry.captureException(new Error(`[fetch-user-categories] ${errorMessage}`));
    ...
    if (userData?.error) {
      console.error("[fetch-user-categories][auth] Authentication failed:", userData.error);
    ...
    if (profileError) {
      console.warn("[fetch-user-categories][fetch-profile] Failed to fetch user profile:", {
        error: profileError,
        userId,
      });
    ...
    if (error) {
      console.error("[fetch-user-categories][fetch-categories] Failed to fetch categories:", {
        error,
        userId,
      });
    ...
    console.error("[fetch-user-categories][unexpected-error] Internal server error:", {
      error,
