/**
 * @deprecated Use the v2 App Router endpoint instead: GET /api/v2/tags/fetch-user-tags
...
export default async function handler(request: NextApiRequest, response: NextApiResponse<Data>) {
  ...
  try {
    ...
    if (!userId || isEmpty(userId)) {
      ...
      Sentry.captureException(new Error("[fetch-user-tags][auth] User ID is missing"));
    ...
    console.error("[fetch-user-tags][unexpected-error] Internal server error:", {
      error,
