...
export default async function handler(
  ...
  try {
    ...
    if (
      ...
      const { data: updateProfilePicData, error: updateProfilePicError } = await supabase
        .from(PROFILES)
        .update({
    ...
    if (!isEmpty(profileData) && profileData && isNil(profileData[0]?.user_name)) {
      const newUsername = getUserNameFromEmail(profileData[0].email as unknown as string);
      ...
      if (checkError) {
        console.error("[fetch-user-profile] Error checking username availability:", {
          error: checkError,
      ...
      if (isEmpty(checkData)) {
        ...
        const { data: userNameNotPresentUpdateData, error: updateUsernameError } = await supabase
          .from(PROFILES)
          .update({
        ...
        const { data: updateUniqueUsernameData, error: updateUniqueUsernameError } = await supabase
          .from(PROFILES)
          .update({
    ...
    console.error("[fetch-user-profile] Error occurred after response was sent:", unexpectedError);
    Sentry.captureException(unexpectedError);
  }
}
