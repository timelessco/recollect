...
export default async function handler(
  ...
  try {
    ...
    if (!email || isEmpty(email)) {
      console.error("[fetch-user-profile-pic] Invalid request: Missing email parameter", {
        query: request.query,
      });
      Sentry.captureException(
        new Error("[fetch-user-profile-pic] Invalid request: Missing email parameter"),
    ...
    console.log("[fetch-user-profile-pic] Successfully fetched profile picture", {
      userId,
    });
