  ...
  useResizeObserver(sidePaneRef, (entry) => {
    ...
    requestAnimationFrame(() => {
      if (elementWidth < SIDE_PANE_SNAP_THRESHOLD) {
        sidePaneElement.style.scale = interpolateScaleValue(elementWidth).toString();
        sidePaneElement.style.transform = `translateX(${interpolateTransformValue(
          elementWidth,
        )}px)`;
        sidePaneElement.style.opacity = interpolateOpacityValue(elementWidth).toString();
      } else {
  ...
  return (
    ...
    <Allotment
      ...
      onDragEnd={(values) => {
        ...
        if (sidePaneSize < SIDE_PANE_SNAP_THRESHOLD && sidepaneWidth && sidepaneWidth > 0) {
          // closes the side pane when user is resizing it and side pane is less than snap threshold
          setTimeout(() => allotmentRef?.current?.resize([0, 100]), SIDE_PANE_ANIMATION_DELAY);
          setShowSidePane(false);
        }

        if (sidePaneSize > SIDE_PANE_SNAP_THRESHOLD && sidePaneSize < SIDE_PANE_DEFAULT_WIDTH) {
