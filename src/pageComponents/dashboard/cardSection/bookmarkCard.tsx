...
const BookmarkCardInner = ({
  ...
  const coverOnly = bookmarksInfoValue?.length === 1 && bookmarksInfoValue[0] === "cover";
  const isCreatedByLoggedInUser = isBookmarkOwner(post?.user_id, userId);

  const tags = bookmarksInfoValue?.includes("tags") && !isEmpty(post?.addedTags) && (
    <div
      className={cn(
        "flex items-center",
        isListView ? "mt-[6px] space-x-px max-sm:mt-px" : "flex-wrap space-x-1",
      )}
    >
