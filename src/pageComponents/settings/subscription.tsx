...
export function Subscription() {
	...
	const isCanceled =
		userData?.subscription_status === "canceled" && isSubscribed;
			...
			{isSubscribed ? (
				<div className="rounded-lg border border-gray-100 p-4">
					<p className="text-[14px] leading-[150%] font-medium text-gray-900">
						You&apos;re on the <span className="capitalize">{currentPlan}</span>{" "}
						plan
					</p>
					{isCanceled ? (
						<p className="mt-1 text-[14px] leading-[150%] text-gray-600">
							Your subscription ends on{" "}
							{formatDate(userData?.subscription_current_period_end)}. You can
							resubscribe through the customer portal.
						</p>
					) : (
						<>
							<p className="mt-1 text-[14px] leading-[150%] text-gray-600">
							...
							{userData?.subscription_current_period_end && (
								<p className="mt-1 text-[13px] leading-[150%] text-gray-500">
									Next renewal:{" "}
									{formatDate(userData.subscription_current_period_end)}
								</p>
							)}
						</>
					)}
					...
					<div className="grid grid-cols-2 gap-3">
						{PLANS.map((plan) => (
							...
							>
								...
								<p className="mt-2 text-[13px] leading-[150%] text-gray-600">
									{plan.description}
								</p>
								<button
