...
// ============================================================
// Raw Handlers (handler receives raw request — no validation interception)
// ============================================================

/**
 * Creates a POST handler that passes the raw NextRequest to the handler function.
 * Unlike other factories, this does NOT call parseBody — the handler is responsible
 * for its own validation, service client creation, and all business logic.
 *
 * Use case: Queue workers that need two-stage validation (extract queue identifiers
 * before Zod parse) so storeQueueError can run on validation failure paths.
 *
 * The factory provides only:
 * 1. `.config` attachment for OpenAPI scanner compatibility
 * 2. Outer try-catch with Sentry capture via apiError for truly unexpected errors
 *
 * Note: inputSchema and outputSchema are passed through to .config for OpenAPI only —
 * there is no runtime validation of input or output by the factory itself.
 */
interface RawHandlerConfig<TInput, TOutput> {
...
export const createRawPostHandler = <TInput, TOutput>(
  config: RawHandlerConfig<TInput, TOutput>,
): HandlerFn => {
  const { handler, inputSchema, outputSchema, route } = config;
  ...
  fn.config = { factoryName: "createRawPostHandler", inputSchema, outputSchema, route };
