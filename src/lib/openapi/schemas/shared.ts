...
export const ArchiveItemSchema = registry.register(
  ...
  z.object({
    ...
    failure_reason: z.string().nullable().meta({ description: "Error message if import failed" }),
...
export const ImportStatusSchema = registry.register(
  "ImportStatus",
  z.object({
    archived: z.int().meta({ description: "Number of successfully archived imports" }),
    archives: z.array(ArchiveItemSchema).meta({ description: "Individual archive item records" }),
    pending: z.int().meta({ description: "Number of imports waiting to be processed" }),
  }),
);
...
export const ImportRetryInputSchema = registry.register(
  ...
  z.union([
    ...
    z
      .object({
        all: z.literal(true).meta({ description: "Set to true to retry all failed imports" }),
      })
      .strict(),
  ]),
);
...
export const ImportRetryOutputSchema = registry.register(
  "ImportRetryOutput",
  z.object({
    requested: z.int().optional().meta({ description: "Number of messages originally requested" }),
    requeued: z.int().meta({ description: "Number of messages requeued for retry" }),
  }),
);
