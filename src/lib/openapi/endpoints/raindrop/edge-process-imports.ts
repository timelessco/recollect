...
export function registerEdgeProcessRaindropImports() {
	registry.registerPath({
		method: "get",
		path: "/process-raindrop-imports",
		servers: edgeFunctionServers,
		tags: ["Raindrop"],
		summary: "Health check for Raindrop import worker",
		description:
					...
					"application/json": {
						...
						examples: {
							"health-check": {
								...
								value: { status: "ok", queue: "raindrop_imports" },
							},
						},
					},
				},
	...
	registry.registerPath({
		...
		servers: edgeFunctionServers,
		tags: ["Raindrop"],
		summary: "Process Raindrop import queue",
		description:
			"Processes pending pgmq messages in the Raindrop imports queue. Each invocation drains one batch. Requires a Supabase service role token — not a user JWT.\n\n**Note:** This endpoint runs as a Supabase Edge Function, not under `/api`. Set the service role key as Bearer token in Scalar's Auth panel.",
