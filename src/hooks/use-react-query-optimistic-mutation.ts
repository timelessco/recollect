...
>({
  ...
  return useReactQueryMutation<TData, TError, TVariables, RollbackFn>({
    ...
    onMutate: async (variables: TVariables) => {
      ...
      for (const { getQueryKey, updater: additionalUpdater } of additionalOptimisticUpdates) {
        const dynamicKey = getQueryKey(variables);
        if (dynamicKey) {
      ...
      const rollback = (() => {
        ...
        if (secondaryQueryKey && secondarySnapshot !== undefined) {
          queryClient.setQueryData<TCacheData>(secondaryQueryKey, secondarySnapshot);
        }
    ...
    onSettled: (data, error, variables, rollback) => {
      ...
      if (!error && rollback?.capturedSecondaryKey && !rollback?.skipSecondaryInvalidation) {
