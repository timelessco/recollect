...
export const MoveBookmarkToTrashInputSchema = z.object({
  data: z
    ...
    .max(1000, { error: "At most 1000 bookmarks per request" })
    .meta({ description: "Array of bookmark objects to move to trash (max 1000)" }),
  isTrash: z.boolean({ error: "isTrash must be a boolean" }).meta({
    description: "True to move to trash, false to restore from trash",
  }),
});

export type MoveBookmarkToTrashInput = z.infer<typeof MoveBookmarkToTrashInputSchema>;
...
export type MoveBookmarkToTrashOutput = z.infer<typeof MoveBookmarkToTrashOutputSchema>;
