...
export const GET = createGetApiHandlerWithAuth({
  handler: async ({ data, route, supabase, user }) => {
    ...
    console.log(`[${route}] API called:`, { category_id, from, sortValue, userId });
    ...
    if (categoryCondition) {
      ...
      try {
        ...
        return apiError({
          error: error instanceof Error ? error : new Error("Category access check failed"),
          extra: { category_id },
    ...
    if (bookmarkError) {
      return apiError({
        error: bookmarkError,
        extra: { category_id, from, sortValue },
    ...
    if (!bookmarkData) {
      return apiError({
        error: new Error("Bookmarks data is null"),
        extra: { category_id },
        message: "Failed to fetch bookmarks",
        operation: "fetch_bookmarks",
        route,
