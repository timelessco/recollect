...
- **Response components**: `ValidationError` (400), `Unauthorized` (401), `InternalError` (500) — all registered as `$ref` in `generate-openapi.ts`. The merge script resolves `$ref` responses inline when `additionalResponses` overrides the description
...
- New supplement files must be exported from `src/lib/openapi/endpoints/<domain>/index.ts` barrel — the merge script auto-discovers them
