...
async function processEnrichBookmark(
  ...
  if (queueError) {
    ...
    console.error(`[twitter-worker] AI queue error for msg ${msg_id}:`, queueError);
  ...
  if (deleteError) {
    console.error(`[twitter-worker] Delete failed for msg ${msg_id}:`, deleteError);
...
async function processLinkBookmarkCategory(
  ...
  const { error: rpcError } = await supabase.rpc("link_twitter_bookmark_category", {
    p_url: linkMsg.url,
  ...
  if (rpcError) {
    ...
    console.error(`[twitter-worker] Link RPC error for msg ${msg_id}:`, rpcError);
    return { type: "retry", reason: "rpc_error" };
  }

  console.log(`[twitter-worker] Processed link_bookmark_category msg ${msg_id}`);
  return { type: "processed" };
}
...
async function processMessage(
  ...
  if (read_ct > MAX_RETRIES) {
    ...
    Sentry.captureException(new Error(`Twitter import failed after ${MAX_RETRIES} retries`), {
    ...
    if (archiveError) {
      console.error(`[twitter-worker] Archive failed for msg ${msg_id}:`, archiveError);
    ...
    console.log(`[twitter-worker] Archived msg ${msg_id}: max_retries_exceeded`);
  ...
  if (payload.type === "enrich_bookmark") {
    ...
    console.error(`[twitter-worker] Unknown message type for msg ${msg_id}:`, unknownType);
