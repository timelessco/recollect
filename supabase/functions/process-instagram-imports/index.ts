...
function extractCollectionNames(collectionNames: unknown): string[] {
  ...
  return collectionNames.filter((name): name is string => typeof name === "string");
}
...
async function processMessage(
  ...
  if (read_ct > MAX_RETRIES) {
    ...
    Sentry.captureException(new Error(`Instagram import failed after ${MAX_RETRIES} retries`), {
    ...
    if (archiveError) {
      console.error(`[instagram-worker] Archive failed for msg ${msg_id}:`, archiveError);
    ...
    console.log(`[instagram-worker] Archived msg ${msg_id}: max_retries_exceeded`);
