...
// Keep in sync with src/utils/constants.ts CHROME_BOOKMARK_IMPORTS_QUEUE
const QUEUE_NAME = "chrome_bookmark_imports";
const BATCH_SIZE = 15;
...
interface QueueMessage {
  ...
  message: {
    ...
    url: string;
    ogImage: string | null;
    chrome_folder_name: string | null;
    user_id: string;
...
async function fetchFavicon(url: string): Promise<string | null> {
  try {
    ...
    const res = await fetch(`https://www.google.com/s2/favicons?sz=128&domain_url=${hostname}`, {
...
async function sanitizeBookmark(url: string, ogImage: string | null): Promise<SanitizedData> {
...
async function processMessage(
  ...
  if (read_ct > MAX_RETRIES) {
    ...
    const archiveReason = lastError ? `max_retries_exceeded: ${lastError}` : "max_retries_exceeded";

    Sentry.captureException(
      new Error(`Chrome bookmark import failed after ${MAX_RETRIES} retries`),
      {
        extra: { msg_id, bookmark, read_ct, lastError },
        tags: { operation: "chrome_bookmark_import_archived" },
      },
    );
    ...
    console.log(`[chrome-bookmark-worker] Archived msg ${msg_id}: max_retries_exceeded`);
  ...
  const { error: rpcError } = await supabase.rpc("process_chrome_bookmark", {
    p_bookmark_id: bookmark.bookmark_id,
    p_user_id: bookmark.user_id,
    p_category_name: bookmark.chrome_folder_name,
  ...
  if (rpcError) {
    ...
    console.error(`[chrome-bookmark-worker] RPC error for msg ${msg_id}:`, rpcError);
    return { type: "retry", reason: "rpc_error" };
  }

  console.log(`[chrome-bookmark-worker] Processed msg ${msg_id} successfully`);
...
Deno.serve(async (req) => {
  ...
  console.log("[chrome-bookmark-worker] Request received");

  // Verify service role JWT
  const authHeader = req.headers.get("Authorization");
  if (!authHeader?.startsWith("Bearer ")) {
    console.error("[chrome-bookmark-worker] Auth failed: missing Bearer token");
    return Response.json(
      { error: "Unauthorized" },
      {
        status: 401,
  ...
  if (token !== serviceRoleKey) {
    console.error("[chrome-bookmark-worker] Auth failed: invalid service role key");
    return Response.json(
      { error: "Unauthorized" },
      {
        status: 401,
  ...
  try {
    ...
    if (readError) {
      console.error("[chrome-bookmark-worker] Queue read error:", readError);
      throw readError;
    }

    if (!messages || messages.length === 0) {
      console.log("[chrome-bookmark-worker] Queue empty");
      return Response.json(
    ...
    if (invalidMessages.length > 0) {
      const archiveResults = await Promise.allSettled(
        invalidMessages.map(async (msg: unknown) => {
          ...
          if (error) {
            Sentry.captureException(new Error("Failed to archive invalid message"), {
              extra: {
                msgId,
                error,
      ...
      Sentry.captureMessage(`Archived ${archivedInvalid} invalid queue messages`, {
        extra: {
          invalidCount,
          archivedInvalid,
    ...
    console.log(
      `[chrome-bookmark-worker] Completed: processed=${processed}, archived=${archived}, skipped=${skipped}, retry=${retry}`,
    );
